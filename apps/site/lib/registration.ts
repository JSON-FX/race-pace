...
const REG_SELECT =
  "id,status,total_amount,ticket_token,org_id,event_id,organizations(name),events(name,status,event_date,original_date,status_note,hero_image_url,inclusions,registration_closes_at),categories(label,distance_km,base_price),payments(checkout_url,created_at,method,amount,platform_fee,net_to_org,provider,provider_ref,status)";
