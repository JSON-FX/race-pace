...
const ultra = { ...category, id: "00000000-0000-4000-8000-000000000006", label: "30K Ultra", code: "30k", distance_km: 30, base_price: 18000 } satisfies CategoryRow;
...
describe("GroupRegister", () => {
  it("requires each selected participant's own waiver acceptance and reserves one order", async () => {
    ...
    expect(reserve).toHaveBeenCalledWith(expect.objectContaining({
      participants: expect.arrayContaining([
        expect.objectContaining({ participant_passport_id: ids[0], category_id: category.id, waiver_acceptance_method: "signed_in_self" }),
        expect.objectContaining({ participant_passport_id: ids[1], category_id: category.id, waiver_acceptance_method: "participant_on_helper_device" }),
      ]),
    }));
  ...
  it("blocks an incomplete guest and does not reserve", async () => {
    render(<GroupRegister userId="booker" initialCategory={category} categories={[category]} event={event} passports={[{ ...passports[1], contact_number: null }]} addons={[]} fields={[]} waiver={waiver} />);
  ...
  it("submits each selected Passport with its own category", async () => {
    reserve.mockResolvedValue({ order_id: "order-mixed", status: "pending" });
    sessionStorage.setItem(`rp:group:booker:${event.id}`, JSON.stringify({
      key: "00000000-0000-4000-8000-000000000007",
      selected: ids,
      lines: {
        [ids[0]]: { categoryId: category.id, addonIds: [], shirtSize: "", values: {}, accepted: false },
        [ids[1]]: { categoryId: ultra.id, addonIds: [], shirtSize: "", values: {}, accepted: false },
      },
    }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "Reserve 2 places" }));
    await waitFor(() => expect(reserve).toHaveBeenCalledOnce());
    expect(reserve).toHaveBeenCalledWith(expect.objectContaining({ participants: expect.arrayContaining([
      expect.objectContaining({ participant_passport_id: ids[0], category_id: category.id }),
      expect.objectContaining({ participant_passport_id: ids[1], category_id: ultra.id }),
    ]) }));
  });

  it("stops another participant from selecting a category with no remaining capacity", async () => {
    const nearlyFull = { ...category, slots_total: 1 };
