 ...
 return <><SiteHeader /><main className="mx-auto max-w-2xl px-6 py-12">
  ...
  {!data?.length ? <p className="mt-6">No assisted bookings yet.</p> : <ul className="mt-6 space-y-4">{data.map(r => {
   ...
   return <li key={r.id} className="rounded-xl border p-4">
    ...
    {r.status === "pending" || r.status === "paid" ? <Link className="mt-3 inline-block underline" href={r.status === "paid" ? `/ticket/${r.id}` : r.booking_order_id ? `/group/order/${r.booking_order_id}` : `/pay/${r.id}`}>{r.status === "paid" ? "View or print participant ticket" : "Continue payment"}</Link> : null}
   </li>;
  })}</ul>}
