...
export function RefundNotice({ policy, retention }: { policy?: string | null; retention?: number | null }) {
  ...
  const message = policy === "none"
    ...
    : policy === "full"
      ? "If you cancel, your refund excludes payment processing fees and any Race Pace service fee. There is no additional organizer cancellation fee."
      : policy === "flat_fee" && knownRetention
        ? `If you cancel, your refund excludes payment processing fees, any Race Pace service fee, and an organizer cancellation fee of up to ${formatPeso(retention)}. The refund cannot be less than zero.`
    ...
    <section aria-label="Refund policy" className="mt-6 rounded-xl border border-border bg-secondary px-5 py-4">
      ...
      {policy === "full" || policy === "flat_fee" ? (
        <p className="mt-2 text-[13px] leading-relaxed text-muted-foreground">
          Processing and service fees are retained even when the organizer covers them in the ticket price.
