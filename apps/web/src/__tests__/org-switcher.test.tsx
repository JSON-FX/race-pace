...
import { pickActiveOrg } from "../lib/orgContext";
...
describe("pickActiveOrg", () => {
  ...
  it("ignores a stored org that is no longer available", () => {
    // The case that motivated validating rather than trusting: an org is
  ...
  it("pins to the single org when the caller suppresses the stored preference", () => {
    // How the provider calls it for an account that cannot switch: passing
    // stored=null means a leftover preference from a super-admin session on
    // the same browser can never move an org admin off their own org.
