  ...
  return useQuery<EditorData | null>({
    ...
    queryFn: async () => {
      const ev = await supabase.from("events")
        .select("id,org_id,name,city_psgc_code,region_name,province_name,city_name,venue,event_date,end_date,flag_off,status,discipline,elevation_gain_m,cutoff_hours,description,hero_image_url,gallery,schedule")
        .eq("id", id!).single();
