...
export type EditorCategory = {
  id: string; code: string; label: string; distance_km: number | null; base_price: number; slots_total: number; slots_taken: number;
  ...
  return useQuery<EditorData | null>({
    ...
    queryFn: async () => {
      const ev = await supabase.from("events")
        .select("id,org_id,name,city_psgc_code,region_name,province_name,city_name,venue,event_date,end_date,flag_off,status,discipline,elevation_gain_m,cutoff_hours,description,hero_image_url,gallery,schedule,inclusions")
        .eq("id", id!).single();
