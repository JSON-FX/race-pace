...
export default function RecoveryPage() {
  ...
  return <main className="grid min-h-dvh place-items-center bg-muted p-6">
    <Card className="w-full max-w-sm rounded-xl shadow-lg"><CardContent className="space-y-4 px-6 py-7">
      ...
      {state === "done" && <><p role="status">Your password has been updated.</p>{error && <p role="alert">{error}</p>}<Link className="block text-sm underline" href="/login">Sign in with your new password</Link></>}
