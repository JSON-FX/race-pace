-- Guest user_id is NULL: ordinary <> would fail open for unrelated callers.
CREATE OR REPLACE FUNCTION public.update_registration_fields_tx(p_registration_id uuid, p_changes jsonb)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  if v_reg.user_id is distinct from v_actor and v_reg.booked_by_user_id is distinct from v_actor and not v_is_admin then return 'forbidden'; end if;
...
$function$;
revoke all on function public.update_registration_fields_tx(uuid,jsonb) from public,anon;
grant execute on function public.update_registration_fields_tx(uuid,jsonb) to authenticated,service_role;
