-- Partial-refund rows appeared in the registration list but vanished from its
-- cards. Count retained paid registrations and actual refunds, matching payments.
...
as $$
  select
    ...
    coalesce(sum(v.total_amount::bigint - v.refunded_amount) filter (where v.payment_status in ('paid', 'partially_refunded')), 0)::bigint    as gross_cents,
