...
const mocks = vi.hoisted(() => ({ createCheckout: vi.fn(), payment: {} as Record<string, unknown>, bound: true }));
vi.mock("../functions/_shared/supabase.ts", () => ({ serviceClient: () => ({
  ...
  from: (table: string) => table === "registrations" ? {
    ...
    select: () => ({ eq: () => ({ single: async () => ({ data: mocks.payment }), maybeSingle: async () => ({ data: mocks.payment }) }) }),
    update: () => ({ eq: () => ({ eq: () => ({ is: () => ({ select: () => ({ maybeSingle: async () => ({
      data: mocks.bound ? { provider_ref: "cs_recovered", checkout_url: "https://checkout.paymongo.com/recovered" } : null,
    }) }) }) }) }) }),
  } : undefined,
}) }));
...
beforeEach(() => {
  ...
  mocks.payment = {
    ...
    checkout_provider_managed_fee: false, amount: 10000, status: "pending",
  ...
  mocks.createCheckout.mockResolvedValue({ providerRef: "cs_recovered", checkoutUrl: "https://checkout.paymongo.com/recovered" });
});
...
describe("lost PayMongo checkout response recovery", () => {
  it("reuses the frozen provider request and binds the recovered session", async () => {
    const response = await request();
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ checkout_url: "https://checkout.paymongo.com/recovered" });
    expect(mocks.createCheckout).toHaveBeenCalledWith(mocks.payment.checkout_request);
  });
  ...
  it("refuses recovery after the provider idempotency window", async () => {
    mocks.payment.created_at = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
    const response = await request();
  ...
  it("accepts the same provider session bound by a concurrent retry", async () => {
    mocks.bound = false;
    mocks.createCheckout.mockImplementation(async () => {
      mocks.payment.provider_ref = "cs_recovered";
      mocks.payment.checkout_url = "https://checkout.paymongo.com/recovered";
      return { providerRef: "cs_recovered", checkoutUrl: "https://checkout.paymongo.com/recovered" };
    });
    const response = await request();
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ checkout_url: "https://checkout.paymongo.com/recovered" });
  ...
  it("refuses an unrelated provider session bound during recovery", async () => {
    mocks.bound = false;
    mocks.createCheckout.mockImplementation(async () => {
      mocks.payment.provider_ref = "cs_other";
      mocks.payment.checkout_url = "https://checkout.paymongo.com/other";
      return { providerRef: "cs_recovered", checkoutUrl: "https://checkout.paymongo.com/recovered" };
    });
    const response = await request();
    expect(response.status).toBe(503);
