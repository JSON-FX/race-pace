...
Fieldnotes event workflow pilot: [design and source sync](./specs/2026-09-25-fieldnotes-event-pilot.md) and [implementation plan](./plans/2026-09-25-fieldnotes-event-pilot.md). The public race list and admin event directory are deployed on staging through PR #120. The catalog spacing, organizer-avatar, and remaining-slots follow-up is deployed through PR #121. Card alignment and the larger image-corner avatar are deployed through PR #123. A correction for long titles in compact card rows awaits staging review. Runner event detail remains at the staging baseline.
