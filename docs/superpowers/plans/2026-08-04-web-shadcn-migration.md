...
> **Corrections applied during execution.** The `Sidebar.tsx` listing below is the pre-review draft and under-specifies four things the handover required; the shipped code fixes all four. (1) The nav label `<span>` needs the same active/inactive colour split as its icon — `text-sidebar-accent-foreground` vs `text-muted-foreground` — otherwise it inherits near-black from the Sidebar root and inactive rows lose their muted treatment. (2) Both `<SidebarGroupLabel>`s need `className="text-[11px] font-semibold tracking-wide"`; the primitive defaults to 12px/500 unspaced. (3) The footer "Sign out" `<Button>` needs `text-[12px] font-semibold`; `size="sm"` does not override the base `text-sm font-medium`. (4) The CLI-generated `ui/sonner.tsx` imports `useTheme` from `next-themes`, which this app does not use — with no provider mounted it pins the toast theme to `"system"` forever, so toasts follow `prefers-color-scheme` instead of the app's `.dark` class. It was rewritten to track `<html>`'s class via a `MutationObserver` (reading the initial value at mount, disconnecting on unmount) and `next-themes` was removed from the dependencies. `ui/sonner.tsx` is a wrapper rather than a Radix primitive, so editing it is a deliberate one-file exception to the no-hand-editing rule.
