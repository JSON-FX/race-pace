...
2. **Access = this Mac only.** ~~admin.racepace.lan resolves via a single `/etc/hosts` entry; plain HTTP; no proxy; Docker publishes host `:80`.~~ **[REVISED at execution — see the plan.]** The machine already runs **dnsmasq** (`*.lan` → 127.0.0.1) and a shared **Traefik** proxy on `dev-net` with an mkcert wildcard `*.lan` cert. So the `web` container joins `dev-net` with Traefik labels for `Host(admin.racepace.lan)` (http→https redirect, `tls=true`, port 5173) and binds **no host port** — served at **`https://admin.racepace.lan`** with a trusted cert, **no `/etc/hosts` needed**, mirroring the existing `.lan` sites (e.g. `runwithpoint.lan`). HTTPS is available now (which Plan 12's camera needs). `docs/plans/09-admin-foundation.md` §Global Constraints + Task 4 is authoritative for the mechanism.
