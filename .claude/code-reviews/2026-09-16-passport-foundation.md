# PIV review — Passport foundation checkpoint

Scope: new Passport migration, shared/Deno contracts, tests and this checkpoint's documentation. Prior deployment/procurement documentation is preserved and excluded from implementation review.

## Review result

No technical defects found in the additive foundation after reading the complete new implementation and tests. Verified explicit grants, restricted ownership columns, row-level isolation, retry collision handling, revoked-manager denial and compatibility protection. No assisted rows can enter the legacy checkout path.

This is not approval of the full assisted-registration feature: its editor, server completeness gate, waiver policy, participant checkout, claims and operational consumers remain unimplemented.

## Validation limitations

- Shared standalone typecheck fails in unchanged photo.ts (seven errors).
- One selected legacy fake-checkout replay test fails because PayMongo mode disables that endpoint (404). Seven other selected provider checks passed.
- Production builds, browser flow and hosted rollout are pending.
- Existing increment_slot/decrement_slot security advisor warnings remain outside this checkpoint.

These limitations are recorded in the implementation report; the full release gate is not passing.

## Second checkpoint review

Reviewed the editor, profile integration, shipping queries/control, mirrored validators, two new migrations and tests. Corrected ambiguous region-wide city choices: without a selected province, only cities without a province are offered. Checked explicit grants and geographic foreign keys. Verified self-profile compatibility writes do not copy managed participant details or overwrite bib names.

380 site tests, 476 backend/shared tests and both app typechecks passed. In-app browser verified saved self/managed records and shipping persistence. No blocking defects found within this editor checkpoint. Current-reference PSGC refresh and complete registration integration remain release work; do not characterize the application as ready for assisted checkout.

## Third checkpoint review

Reviewed self-checkout guard placement, canonical snapshot assignment, account-scoped drafts and both client/server error handling. Existing live bookings return before completeness enforcement; incomplete new bookings cannot create a pending row. No new authenticated INSERT permission is granted on registrations. Preserved the partial unique index and verified concurrent insertion/expired-entry tests with complete fixture Passports.

382 site tests, 477 backend/shared tests and site typecheck passed. Browser confirmed saved identity review through the confirmation screen. Explicitly not an assisted-checkout approval: the other-Passport request remains refused and user_id semantics are unchanged.
